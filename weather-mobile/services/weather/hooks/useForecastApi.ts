...
export type TodaysForecast = Awaited<ReturnType<ReturnType<typeof useForecastApi>["getTodaysForecast"]>>;

export const useForecastApi = () => {
  const [isLoading, setIsLoading] = useState(false);

  const getTodaysForecast = useCallback(async ({ city }: { city: string }) => {
    setIsLoading(true);
